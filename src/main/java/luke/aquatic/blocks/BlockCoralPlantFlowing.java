...
public class BlockCoralPlantFlowing extends BlockFluidFlowing {
	int numAdjacentSources = 0;
	boolean[] isOptimalFlowDirection = new boolean[4];
	int[] flowCost = new int[4];

	public BlockCoralPlantFlowing(String name, int openIds, Material water) {
	...
	private void setFluidStill(World world, int x, int y, int z) {
		...
		world.setBlockAndMetadata(x, y, z, AquaticBlocks.coralPlantStill.id, meta);
	...
	private int calculateFlowCost(World world, int i, int j, int k, int l, int i1) {
		...
		for (int k1 = 0; k1 < 4; ++k1) {
			...
			if (l >= 4 || (k2 = this.calculateFlowCost(world, l1, j, j2, l + 1, k1)) >= j1) continue;
			j1 = k2;
		}
		return j1;
	}
	...
	private boolean[] getOptimalFlowDirections(World world, int i, int j, int k) {
		for (int l = 0; l < 4; ++l) {
			...
			if (this.blockBlocksFlow(world, j1, j, j2) || world.getBlockMaterial(j1, j, j2) == this.blockMaterial && world.getBlockMetadata(j1, j, j2) == 0)
				continue;
			this.flowCost[l] = !this.blockBlocksFlow(world, j1, j - 1, j2) ? 0 : this.calculateFlowCost(world, j1, j, j2, 1, l);
		}
